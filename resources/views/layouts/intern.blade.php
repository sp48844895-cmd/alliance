...
<head>
    ...
    <title>@yield('title', 'Intern') · ABC Chhattisgarh</title>
...
@php
    $sections = [
        ['heading' => 'Overview', 'links' => [
            ['route' => 'intern.dashboard', 'icon' => 'bi-grid-1x2', 'label' => 'Dashboard', 'wildcard' => 'intern.dashboard'],
        ]],
...
<div class="flex min-h-screen">
    ...
    <aside data-admin-sidebar class="fixed lg:sticky top-0 z-40 h-screen w-[280px] -translate-x-full lg:translate-x-0 [&.is-open]:translate-x-0 transition-transform duration-300 bg-[var(--color-paper)] border-r border-[var(--color-line)] flex flex-col">
        ...
        <div class="px-5 py-5 border-b border-[var(--color-line)]">
            <div class="font-display text-lg text-[var(--color-ink-2)] font-medium">Intern portal</div>
        ...
        <div class="p-4 border-t border-[var(--color-line)] text-xs text-[var(--color-mute)]">
            Signed in as <span class="font-semibold text-[var(--color-ink-2)]">{{ auth()->user()->fname ?? 'Intern' }}</span>
        </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col min-w-0">
        ...
        <main class="flex-1 p-4 lg:p-8">
            @if(session('work_log_status'))
                <div class="alert alert-success mb-6 text-sm">
                    <i class="bi bi-check-circle"></i>
                    <span>{{ session('work_log_status') }}</span>
                </div>
            @endif
            @yield('content')
        </main>
