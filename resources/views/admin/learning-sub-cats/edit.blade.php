...
@section('breadcrumb')
    <a href="{{ route('admin.learning-sub-cats.index') }}">Subcategories</a>
...
@section('topbar_actions')
    <a href="{{ route('admin.learning-sub-cats.index') }}" class="btn btn-ghost">
...
@section('content')
    <form method="POST" action="{{ route('admin.learning-sub-cats.update', $category->id) }}" class="card p-6 max-w-2xl">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="md:col-span-2">
                ...
                <select name="parent_id" id="parent_id" class="select" required>
                    @foreach ($mainCategories as $main)
                        <option value="{{ $main->id }}" {{ (string) old('parent_id', $category->parent_id) === (string) $main->id ? 'selected' : '' }}>
                            {{ $main->cat_name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="flex items-center justify-end gap-2 mt-6">
            <a href="{{ route('admin.learning-sub-cats.index') }}" class="btn btn-ghost">Cancel</a>
