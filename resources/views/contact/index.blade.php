...
@section('title', $metaTitle ?? 'Contact Us · ChhattisgarhABC')
@section('meta_description', $metaDescription ?? 'Contact ChhattisgarhABC in Raipur, Chhattisgarh for partnerships, volunteering, resources, events and social behaviour change communication collaboration.')
    ...
    <div class="container-x">
      <div class="contact-main-grid">
        <div class="contact-form-card" data-aos="fade-up">
          <span class="chapter"><b>{{ $sections['contact_form']['chapter'] ?? '01' }}</b> · Message</span>
          <h2 id="contact-form-h">
            ...
            @else
              {{ $sections['contact_form']['title'] ?? 'Send a message to the alliance.' }}
            @endif
          </h2>
          <p>
            @if($requiresAccount)
            ...
            @else
              {{ $sections['contact_form']['description'] ?? 'Your email address will not be published. Share a few details and the team will respond by email.' }}
            @endif
          </p>
        ...
        <aside class="contact-aside" data-aos="fade-up" data-aos-delay="120" aria-label="Contact details">
          @foreach ($sections['contact_cards']['cards'] ?? [] as $card)
            @if (($card['icon'] ?? '') === 'phone')
              @continue
            @endif
    ...
    <div class="container-x">
      <div class="contact-map-grid">
        <div class="contact-map-copy" data-aos="fade-up">
          <span class="chapter"><b>{{ $sections['contact_map']['chapter'] ?? '03' }}</b> · Location</span>
          <h2 id="contact-map-h">{{ $sections['contact_map']['title'] ?? 'Based in Raipur, connected across Chhattisgarh.' }}</h2>
          <p>{{ $sections['contact_map']['description'] ?? 'The alliance works with local members, partners and community networks across districts. Use the map to locate Raipur or open it in Google Maps.' }}</p>
          <a class="btn btn-ghost" href="{{ $sections['contact_map']['map_url'] ?? 'https://www.google.com/maps/search/?api=1&query=Raipur%2C%20Chhattisgarh' }}" target="_blank" rel="noopener">{{ $sections['contact_map']['button_text'] ?? 'Open map' }}</a>
        </div>

        <div class="contact-map-card" data-aos="fade-up" data-aos-delay="120">
          <iframe title="Map showing Raipur, Chhattisgarh" src="{{ $sections['contact_map']['iframe_src'] ?? 'https://www.google.com/maps?q=Raipur%2C%20Chhattisgarh&output=embed' }}" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
        </div>
      </div>
    </div>
  </section>
